...
SELECT  AUDITORIUM.AUDITORIUM_TYPE [Тип аудитории], 
		...
		count(*) [Количество]
			FROM AUDITORIUM 
			GROUP BY ROLLUP (AUDITORIUM.AUDITORIUM_TYPE, AUDITORIUM.AUDITORIUM_NAME);
-----------------------------------------------------------
SELECT FACULTY.FACULTY_NAME [Факультет], 
		GROUPS.IDGROUP [Номер группы],
		count(*) [Количество]
   FROM FACULTY
		Inner Join GROUPS ON FACULTY.FACULTY = GROUPS.FACULTY
		Inner Join STUDENT on STUDENT.IDGROUP = GROUPS.IDGROUP
   GROUP BY FACULTY_NAME, GROUPS.IDGROUP
   ORDER BY FACULTY_NAME, GROUPS.IDGROUP
SELECT  COUNT(*) [Всего в университете]
		FROM STUDENT
